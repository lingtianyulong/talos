...
void GraphicsView::paintEvent(QPaintEvent *event) {
  ...
  QRectF rect = viewport()->rect().adjusted(0.5, 0.5, -0.5, -0.5);
  QPen pen(QColor(255, 255, 255, 160)); // 半透明白色
  pen.setWidth(1);
...
void GraphicsView::drawMyBackground(QPainter *painter, const QRectF &rect) {
  ...
  painter->fillRect(rect, _backBrush);
}
...
bool GraphicsView::isPosInImage(const QPointF &pos) const {
  return !_pixmap.isNull() && pos.x() >= 0 && pos.x() <= _pixmap.width() &&
         pos.y() >= 0 && pos.y() <= _pixmap.height();
}
