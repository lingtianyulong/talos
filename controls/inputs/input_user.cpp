...
InputUser::InputUser(QWidget *parent) : InputBase(parent) { InputUser::init(); }

InputUser::~InputUser() {}

void InputUser::init() {
  InputBase::init();
  QFont font = utils::FontHelper::getFont(":/font/iconfont.ttf", 14);
  if (_label != nullptr) {
    _label->setFont(font);
    _label->setStyleSheet("color: #666666; border: none");
