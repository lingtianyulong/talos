...
namespace talos::sys::config {
  ...
  bool RustSysConfig::loadConfig(std::string_view config_path) {
    if (!std::filesystem::exists(config_path)) {
      Logger::Error(std::format("当前加载的系统的配置文件路径不存在, 配置文件路径为:{}", config_path));
      return false;
    }
  ...
  void RustSysConfig::setWinWidth(int winWidth) {
    _winWidth = winWidth;
  }

  void RustSysConfig::setWinHeight(int winHeight) {
    _winHeight = winHeight;
  }

  std::optional<int> RustSysConfig::getWinWidth() const {
    return _winWidth;
  }

  std::optional<int> RustSysConfig::getWinHeight() const {
    return _winHeight;
  }
