...
void LoginDialog::handleLogin(const QString& username, const QString& password) {
    ...
    if (obj.contains("success") && obj["success"].toBool() == true) {
        ...
        if (!token.isEmpty()) {
            if (utils::secure_store_set("talos", "access_token", token.toUtf8().constData()) != 0) {
