...
MainWidget::MainWidget(QWidget* parent) : QWidget(parent), ui(new Ui::MainWidget) {
    ...
    QString fontFamily = utils::FontHelper::loadApplicationFont(":/font/iconfont.ttf");
...
bool MainWidget::isInTitleArea(const QPoint& pos) const {
    ...
    rect.setBottomRight(QPoint(this->geometry().right(), this->geometry().top() + 30));
...
void MainWidget::mouseMoveEvent(QMouseEvent* event) {
    if (_isDragging && (event->buttons() & Qt::LeftButton)) {
        if (_isMaximized) {
            ...
            // 3. 根据比例计算还原后的新位置
            int newX = event->globalPosition().toPoint().x() - (_normalGeometry.width() * ratio);
...
void MainWidget::onOpenFile() {
    QString filePath = QFileDialog::getOpenFileName(
        this, tr("打开文件"), "", tr("所有文件 (*.*);;图像文件 (*.png *.jpg);;文本文件 (*.txt)"));
...
void MainWidget::initUI() {
    ...
    centralWidget->setStyleSheet("#centralContentWidget { background: transparent; }");
