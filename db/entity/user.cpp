...
namespace talos::db {
    ...
    User::User(const User &user) {
        // 只能进行深拷贝, 否则, this->_user 与 user._user 会占用相同的内存, 当析构时, 会引起 double free.
        if (user._user) {
    ...
    int UserManager::insertUser(const User &user) {
        ...
        return insert_user(DbUtil::instance().raw(), user.getRustUser()->user_name, user.getRustUser()->password);
    }
